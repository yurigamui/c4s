// functions/api/confirmar-usuario.js
...
export async function onRequestPost(context) {
    ...
    try {
        const { login, codigo } = await request.json();
        const bearerToken = await getValidOAuthToken(env);

        const smartResponse = await fetch('https://api.smartsecurities.com.br/debenturista/auth/confirmar-usuario', {
            method: 'POST',
            headers: {
